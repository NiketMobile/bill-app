...
const EditReligion = () => {
    ...
    const getData = async () => {
        setLoading(true);
        try {
            const response = await apiServices.getUserDoc(userInfo?.uid);

            if (response?.religion?.religion_id) {
                setSelectedId(response?.religion?.religion_id);
            }
            if (response?.other) {
                setOtherValue(response?.religion?.other);
            }

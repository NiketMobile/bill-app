...
const OnboardingO = () => {
    ...
    const getData = async () => {
        ...
        try {
            ...
            console.log('userDoc', JSON.stringify(userDoc?.address_data, null, 2))

            const response = userDoc?.political_affiliation;

            if (response) {
                setSelectedId(response.political_affiliation_id);
            }
            if (response?.other) {
                setOtherValue(response.other);
            }
